...
Information={
Version="v1.0.0",
GitHubOwner="n0namevnnek-web"
},
Default={
Theme="Darker",
UISize=UDim2.fromOffset(480,320),
TabSize=140
},
...
local ar=V[am]
local as=(am._LastSectionContent or T[am].Container)
...
local aw=V[self]
local container = T[self].Container

local ax=E("Frame","Option",container,{
Size=UDim2.new(1,0,0,22),
BackgroundTransparency=1,
ClipsDescendants=false
})

local ay=E("TextButton",ax,{
Size=UDim2.new(1,0,0,22),
BackgroundTransparency=1,
Text="",
AutoButtonColor=false
...
local titleLabel=E("TextLabel",ay,{
TextXAlignment=Enum.TextXAlignment.Left,
TextTruncate=Enum.TextTruncate.AtEnd,
...
local function toggle()
    ...
    -- force layout update
    ax.Size = UDim2.new(1,0,0, isOpen and (22 + contentFrame.AbsoluteSize.Y) or 22)
end

u(ay.Activated, toggle)

-- Make subsequent options parent to this contentFrame by overriding temporarily
-- Store reference so other Add* can use it if we extend later
...
-- Keep a reference on the tab for last section content (for future extension)
self._LastSectionContent = contentFrame
...
function ag:SetBackgroundImage(imageId)
-- imageId can be number or string like "12345" or full "rbxassetid://12345"
-- Prefills with rbxassetid:// so user only needs to pass the number/id
if imageId == nil or imageId == "" then
    local bg = ab:FindFirstChild("BackgroundImage")
    if bg then bg.Visible = false end
    return
end
local id = tostring(imageId)
if not id:find("rbxassetid://") then
    id = "rbxassetid://" .. id:gsub("%D", "") -- strip non digits if needed, but keep simple
end
local bg = ab:FindFirstChild("BackgroundImage")
if bg then
    bg.Image = id
...
local al=`{ak}/Library/refs/heads/main/NNVN-library/main/Utils/Icons.lua`
...
-- Custom Background Image support (user can set via SetBackgroundImage)
local BackgroundImage = E("ImageLabel",ak,{
